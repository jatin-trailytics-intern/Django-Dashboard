...
# sakshi vaval
def package_data(start_date, end_date):
    try:
        ...
        if connection.is_connected():
            ...
            fetched_data = []
            for row in rows:
                roas = float(row['total_sales']) / float(row['total_clicks']) if float(row['total_clicks']) != 0 else 0
                ...
                row_data = {
                    ...
                    'ctr': float(row['total_clicks']) / float(row['total_impressions']) if float(row['total_impressions']) != 0 else 0,
                    'roas': roas,
                }
